...
export const knowledgeGraphState = reactive({
  ...
  saveToStorage() {
    try {
      ...
      localStorage.setItem('knowledgeGraphData', JSON.stringify(dataToSave))
    } catch (error) {
      console.error('保存知识图谱数据到localStorage失败:', error)
    }
  },
  ...
  loadFromStorage() {
    try {
      ...
      console.error('从localStorage恢复知识图谱数据失败:', error)
      this.clearStorage()
    }
    return false
  },
  ...
  clearStorage() {
    try {
      localStorage.removeItem('knowledgeGraphData')
    } catch (error) {
      console.error('清除localStorage中的知识图谱数据失败:', error)
    }
  }
})
